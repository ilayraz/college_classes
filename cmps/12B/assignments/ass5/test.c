...
int isEmpty(Dictionary dict) { return dict->size == 0; }

int size(Dictionary dict) { return dict->size; }

char *lookup(Dictionary dict, char *k) {
  Node node;
  for (node = dict->table[hash(k)]; node != NULL && node->key != k;
       node = node->next)
    ;
  if (node == NULL)
    return NULL;
...
void delete (Dictionary dict, char *k) {
  ...
  int mhash = hash(k);
  Node toDel;
  for (toDel = dict->table[mhash]; strcmp(toDel->key, k) != 0;
       toDel = toDel->next)
    ;
